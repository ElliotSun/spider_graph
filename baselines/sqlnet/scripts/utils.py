...
def load_dataset(dataset_dir, use_small=False):
    ...
    TRAIN_PATH = os.path.join(dataset_dir, "train_spider.json")
...
def print_results(model, batch_size, sql_data, table_data, output_file, schemas, pred_entry, error_print=False,
                  train_flag=False):
    model.eval()
    perm = list(range(len(sql_data)))
    st = 0
